...
import { MatchdayEssentials } from '@/components/dashboard/home/MatchdayEssentials'
import { NextMatchCard } from '@/components/dashboard/home/NextMatchCard'
...
function QuickAction({
  ...
  return (
    <button className='flex items-center gap-3 rounded-2xl border border-border bg-card p-3.5 sm:p-4 text-left transition-all hover:border-primary/30 hover:bg-muted/30'>
    ...
    <div className='space-y-6 p-4 sm:p-6'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
        ...
        >
          ...
          <h1 className='font-heading text-[26px] sm:text-[32px] font-bold leading-[1.05] text-foreground'>
            {DASHBOARD_USER.name}!
          </h1>
          <p className='mt-2 text-[12px] sm:text-[13px] text-muted-foreground'>
            Everything you need for an unforgettable matchday experience.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className='w-full overflow-hidden rounded-2xl border border-border bg-card sm:w-62.5 sm:shrink-0'
        >
          <div className='flex items-start justify-between p-4'>
            ...
            <span
              ...
              style={{ filter: 'drop-shadow(0 2px 10px rgba(255,200,50,0.4))' }}
            >
              🏆
            </span>
          </div>
      ...
      {/* ── Component 1: Next Match Card ── */}
      <NextMatchCard />

      {/* ── Component 2: Matchday Essentials ── */}
      <MatchdayEssentials />

      {/* ── Component 3: Inbox + Rewards ── */}
      <InboxAndRewards />

      {/* Plan Everything banner */}
      <motion.div
        ...
        style={{
          background:
            'radial-gradient(ellipse at 70% 50%, #0e2010 0%, #080c08 100%)',
        }}
      >
        {/* Background image */}
        <div
          ...
          style={{
            backgroundImage:
              'url(https://images.unsplash.com/photo-1431324155629-1a6deb1dec8d?w=400&q=60)',
            backgroundSize: 'cover',
            backgroundPosition: 'center',
          }}
        />
        <div className='pointer-events-none absolute inset-y-0 right-0 w-1/2 bg-linear-to-l from-transparent to-[#080c08]' />
        ...
        <div className='relative z-10 flex flex-col gap-5 p-6 sm:p-7 lg:flex-row lg:items-center'>
          ...
          <div className='max-w-md'>
            ...
            <p className='mt-3 text-[12px] sm:text-[13px] leading-relaxed text-muted-foreground'>
              Book tickets, hotels, flights and stay connected with eSIM — all
              in one place.
            </p>

            <Link
              href='/dashboard/travel'
              className='mt-5 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-heading text-[12px] sm:text-[13px] font-bold text-background transition-opacity hover:opacity-90'
            >
              Explore Travel Options
            </Link>
          </div>

          {/* Icon row — hidden on mobile, shown on lg+ */}
              ...
              {[
                ...
                <div key={label} className='flex'>
                  <div className='flex flex-col items-center gap-2'>
                    <span className='flex h-12 w-12 items-center justify-center rounded-xl border border-primary/30 bg-primary/15 backdrop-blur-sm z-10'>
                      <Icon className='h-5 w-5 text-primary' />
                    </span>
                    <span className='font-mono text-[11px] text-muted-foreground whitespace-nowrap text-center'>
                      {label}
                    </span>
                  </div>
                  {i < 3 && (
          ...
          {/* Mobile-only: compact icon row in 2x2 grid */}
      ...
      <section className='space-y-4'>
        ...
        <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4'>
          ...
          <QuickAction
            ...
            sub='Transfer to another fan'
          />
          <QuickAction
            icon={CalendarPlus}
            label='Add to Calendar'
            sub='Never miss a match'
          />
          <QuickAction
            icon={HelpCircle}
      ...
      <div className='flex items-center justify-center gap-2 py-2'>
        ...
        <p className='font-mono text-[10px] sm:text-[11px] text-muted-foreground'>
          Thanks for being part of the Matchday community!
        </p>
      </div>
    </div>
  )
