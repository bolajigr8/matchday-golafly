...
interface FlightCardProps {
  offer: FlightOffer;
  index: number;
  /** If provided, renders an Add/Added button instead of the "View Deal" navigation link */
  onSelect?: (offer: FlightOffer) => void;
  selected?: boolean;
}

export function FlightCard({ offer, index, onSelect, selected }: FlightCardProps) {
  ...
  const symbol   = ({ EUR: "€", USD: "$", GBP: "£", AED: "د.إ" } as Record<string, string>)[offer.total_currency] ?? offer.total_currency + " ";
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          {firstSeg.marketing_carrier.logo_symbol_url ? (
            <img src={firstSeg.marketing_carrier.logo_symbol_url} alt={airline} className="h-6 w-6 rounded object-contain" />
          ) : (
      ...
      <div className="my-3 flex items-center gap-2">
        <div className="text-center">
          <p className="text-[16px] font-bold leading-none text-foreground">{fmtTime(firstSeg.departing_at)}</p>
          <p className="mt-0.5 font-mono text-[10px] text-muted-foreground">{slice.origin.iata_code}</p>
        </div>
        ...
        <div className="text-center">
          <p className="text-[16px] font-bold leading-none text-foreground">{fmtTime(lastSeg.arriving_at)}</p>
          <p className="mt-0.5 font-mono text-[10px] text-muted-foreground">{slice.destination.iata_code}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-border pt-3">
        <span className={`rounded-full px-2 py-0.5 font-mono text-[10px] font-bold ${totalStops === 0 ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}>
        ...
        {onSelect ? (
          <motion.button
            ...
            className={`rounded-full px-4 py-1.5 text-[12px] font-bold transition-all ${
              selected
                ? "bg-primary text-primary-foreground shadow-[0_0_8px_rgba(183,255,0,0.3)]"
