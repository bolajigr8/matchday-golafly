  ...
  return (
    <div className="flex flex-wrap gap-2">
      {tabs.map(tab => (
        ...
        >
          <p className={`text-[10px] font-mono ${activeSort === tab.id ? "text-primary" : "text-muted-foreground"}`}>{tab.label}</p>
          <p className={`text-[14px] font-bold ${activeSort === tab.id ? "text-primary" : "text-foreground"}`}>€{tab.price}</p>
        </motion.button>
      ))}
    </div>
  );
